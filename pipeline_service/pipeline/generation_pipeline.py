...
class GenerationPipeline:
    ...
    async def startup(self) -> None:
        ...
        if self._pipeline is not None and self._pipeline.coder_v1 is not None:
            logger.info(
                f"Coder v1: {actors.coder_v1.client} | Model: {actors.coder_v1.model} | "
                f"multimodal={actors.coder_v1.multimodal} | ensemble_size={actors.coder_v1.ensemble_size} | "
                f"ensemble_temperature={actors.coder_v1.ensemble_temperature}"
            )
        else:
            logger.info("Coder v1: DISABLED (no second coder configured)")
    ...
    async def run_batch(self, tasks: list[PipelineTask]) -> None:
        ...
        if tasks:
            ...
            if self._pipeline.coder_v1 is not None:
                self._pipeline.coder_v1.seed = batch_seed
