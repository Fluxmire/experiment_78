...
class ActorsConfig(BaseModel):
    ...
    coder_v1: ActorConfig = ActorConfig(
        workers=2, queue_size=8,
        client="openrouter", model="qwen/qwen-2.5-72b-instruct",
        max_tokens=8192,
    )
...
class SettingsConf(BaseSettings):
    ...
    def _validate_actor_clients(self) -> "SettingsConf":
        ...
        # The optional second coder is validated like any other LLM actor, but
        # only once it is actually wired up: it names an enabled client and is
        # configured for >= 1 candidate. This keeps the dormant default (which
        # points at a possibly-absent "openrouter") from failing startup while
        # still catching a real misconfiguration (enabled client, no model).
        cv1 = self.actors.coder_v1
        if (
            cv1.ensemble_size >= 1
            and cv1.client in known
            and self.llm_clients[cv1.client].enabled
        ):
            actor_names.append("coder_v1")
        # Multigen (and the judge bracket) run when the combined ensemble > 1.
        secondary = cv1.ensemble_size if "coder_v1" in actor_names else 0
        if self.actors.coder.ensemble_size + secondary > 1:
